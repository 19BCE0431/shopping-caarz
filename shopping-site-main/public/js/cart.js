//GLOBAL
var products = JSON.parse(localStorage.geItem('cart'));
var cartItems = [];
...
function clean() {
    ...
    `;
    cart_n.innerHTML='';
    document.getElementById("btnBuy").style.dispay="none";
    document.getElementById("btnClean").style.dispay="none";
}

(()=>{
    for(let index=0; index <products.length; index++){
        table.innerHTML+=tableHTML(index);
        total=total+parseInt(products[index].proce);
    }
    table.innerHTML+=`
        <tr>
            <th scope="col"></th>
